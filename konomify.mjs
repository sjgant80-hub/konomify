...
//   1. GRADE + PROVE  — run the benchmark (acg-assessor) and mint a Proof-of-Play.
//   2. THE GATE       — not admissible? STOP. Report what is undercooked and send it back to the
//                       pan. Waste is NOT grown into an organ. This is the honest edge: konomify
//                       does not badge-farm, it digests.
//   3. GRAFT          — admissible? emit an organ manifest so the build can join the living mesh.
//   4. CARD           — admissible? mint a fallkard receipt card of the metabolized build.
//
// konomify adds no new organ. It CONNECTS the ones you already have — acg-assessor (the gut),
// proof-of-play (the absorption wall), the mesh graft (becoming an organ), fallkard-forge (what
// comes out). The stages are injectable, so this file is testable with no sibling tools present;
// the CLI wires the real ones.
//
...
export const KONOMIFY_VERSION = '0.1';
...
export function receiptHtml({ repo, ring, core, hash, benchmark }) {
...
<div style="max-width:520px">
  ...
  <table style="margin-top:16px;border-collapse:collapse">
    ...
    <tr><td style="color:#8b93a7;padding:3px 16px 3px 0">core</td><td>${core}</td></tr>
...
// ── the tract ──────────────────────────────────────────────────────────────
// stages: { prove(repoPath)->Proof-of-Play, forge?(opts)->{png,manifest}, renderCard?(opts)->Buffer }
// Only `prove` is required. Without forge/renderCard the card step is skipped (organ still emitted).
export function konomify(repoPath, { prove, forge, renderCard, ring, provedAt } = {}) {
  ...
  // THE GATE — waste is not grown into an organ.
  if (!proof || proof.admissible !== true) {
    const v = proof?.verdict || {};
    return {
      v: KONOMIFY_VERSION, repo, konomified: false, organ: null, card: null, proof: proof || null,
      undercooked: {
        core: v.core ?? null,
  ...
  return { v: KONOMIFY_VERSION, repo, konomified: true, undercooked: null, proof, organ, card };
}
